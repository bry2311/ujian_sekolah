...
class M_ujian_gabungan_has_soal extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT us.*, s.materi, s.kd, s.soal FROM ujian_gabungan_has_soal us JOIN soalisian s ON us.id_soal = s.id WHERE us.id_ujian = $id AND us.tipe ='Isian' ORDER BY us.no_soal");
		return $query->result();
	}
