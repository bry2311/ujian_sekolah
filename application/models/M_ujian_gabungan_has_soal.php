...
class M_ujian_gabungan_has_soal extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT us.*, s.materi, s.kd, s.soal, s.a, s.b , s.c, s.d, s.e, s.kunci_jawaban, s.kunci_pg, s.checkSoal, s.checkA, s.checkB, s.checkC, s.checkD, s.checkE FROM ujian_gabungan_has_soal us JOIN soalpg s ON us.id_soal = s.id WHERE us.id_ujian = $id AND us.tipe ='Pilihan Ganda' ORDER BY us.no_soal");
		return $query->result();
	}
	...
	{
		$query = $this->db->query("SELECT us.*, s.materi, s.kd, s.soal, s.bobot FROM ujian_gabungan_has_soal us JOIN soalisian s ON us.id_soal = s.id WHERE us.id_ujian = $id AND us.tipe ='Isian' ORDER BY us.no_soal");
		return $query->result();
	}
	...
	{
		$query = $this->db->query("SELECT us.*, s.materi, s.kd, s.soal, s.a, s.b , s.c, s.d, s.e, s.kunci_jawaban, s.kunci_pg, s.gambarSoal FROM ujian_gabungan_has_soal us JOIN soalpg s ON us.id_soal = s.id WHERE us.id_ujian = $id AND us.tipe = 'Pilihan Ganda' ORDER BY us.no_soal");
		return $query->result();
	}
